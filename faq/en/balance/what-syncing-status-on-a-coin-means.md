...
The 'syncing' status indicator on the coin shows that the app tries to synchronize itself with a given blockchain.
...
Keep in mind, app keeps the synchronization process running only when the app remains open. If the app is closed the synchronization process is paused and only resumes when the app is opened again.
